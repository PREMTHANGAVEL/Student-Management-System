#Your code looks good, but there are a couple of minor corrections. Here is the corrected version:
...
class Student:
    ...
    def update(self):
        ...
        cursor = c.cursor()
        cursor.execute("UPDATE Student SET ID=?, NAME=?, AGE=?, DOB=?, GENDER=?, CITY=? WHERE ID=?", (id, name, age, dob, gender, city, selected_item))
        c.commit()
        c.close()
...
#I've corrected the database table name in the `INSERT INTO` statement and added parentheses to the `commit()` and `close()` methods in the `Add` method. This should work correctly for adding, updating, deleting, and clearing entries in your student management system.
